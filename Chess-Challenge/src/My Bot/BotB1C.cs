...
public class BotB1C : IChessBot
{
    ...
    private Dictionary<ulong, (short, Move)> _historyHeuristic = new Dictionary<ulong, (short, Move)>();
    ...
    bool _useHistoryHeuristic = false;
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        if (_useHistoryHeuristic)
            _historyHeuristic.Clear();

        Console.WriteLine("BotB1C finished at depth: " + searchDepth + " in: " + timer.MillisecondsElapsedThisTurn + " milliseconds, " + " with searches: " + _searches + " and quiescnceSearhces: "  + _quiescenceSearches);
        return bestMove;
    }
    ...
    private (List<Move>, int) Search(Board board, int depth, int currentEvaluation, int alpha, int beta, Move pvMove, bool quiescenceSearch, HashSet<Square> capturePieces, int quiescenceDepth)
    {
        ...
        if (_useHistoryHeuristic && _historyHeuristic.TryGetValue(board.ZobristKey, out var storedEntry2) && storedEntry2.Item1 == board.PlyCount)
        {
            bestMove = storedEntry2.Item2;
            //Move ordering with the best move from the transposition table first
            moves = moves.OrderByDescending(move => move.Equals(bestMove) ? 1 : 0).ThenByDescending(move => _pieceValues[(int)move.CapturePieceType]).ToArray();
        }
        else if (_useIterativeDeepening && _useEvaluateMoveOrdering)
        {
        ...
        else if (_useEvaluateMoveOrdering)
        {
            //Standard move ordering
            moves = moves.OrderByDescending(move => EvaluateMove(move, board.IsWhiteToMove)).ToArray();
        }
        else if (_useIterativeDeepening)
        {
            //moves = moves.OrderByDescending(move => move.Equals(pvMove) ? 1 : 0).ThenByDescending(move => _pieceValues[(int)move.CapturePieceType]).ToArray();
        ...
        //_historyHeuristic[board.ZobristKey] = ((short)board.PlyCount, bestMove);
