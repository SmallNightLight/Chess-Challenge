...
public class Bot8 : IChessBot
{
    public Move Think(Board board, Timer timer)
    {
        ...
        string totalString = string.Concat(_positionMaps), endString = "";

        for (int j = 0; j < 384; j += 4)
            endString += totalString.Substring(j, 4) + new string(totalString.Substring(j, 4).Reverse().ToArray());

        for (int o = 0; o <= 383; o++)
            _maps[o] = (int)(progression * _numberValue[int.Parse(endString.Substring(0, 384)[o].ToString())] + (1 - progression) * _numberValue[int.Parse(endString.Substring(384, 384)[o].ToString())]);
    ...
    int[] pieceValues = { 0, 100, 300, 325, 500, 900, 9999 };
    ...
    private int GetRawPositionEvaluation(Board board)
    {
        ...
        foreach (PieceList pieceList in pieces)
            ...
            {
                int value = pieceValues[(int)piece.PieceType] + _maps[((int)piece.PieceType - 1) * 64 + (piece.IsWhite ? piece.Square.Index : 63 - piece.Square.Index)];
